...
uni <- c(
  "LUT",
  "ALL", "TAM", "OUL", "MIT", "KIT", "TSI", "JIO", "AZA", "LUT2", "FRA", "SIT", "BF", "DIM"
)
...
for (cm in RESULTC) {
  ...
  # RESULTB=c(1:1)
  for (bm in RESULTB) {
    ser_file <- sprintf("AB3/SEARCH_%s_%s", uni[cm], year[bm])
    # AB HAKEMISTOON!!!
    load(file = ser_file)
    ...
    if (res$total_results > 0) {
      ...
      for (am in RESULT) {
        ...
        art_file <- sprintf("AB3/AB_%s_%s_%s", uni[cm], year[bm], fetch_file[[1]][2])
        print("===========")
        ...
        if (file.exists(art_file)) {
          ...
          print(fetch_file[[1]][2])
          print(x$content$"abstracts-retrieval-response"$coredata$`dc:identifier`)
          print("== ")
          if (!is.null(x$content$"abstracts-retrieval-response"$coredata$`dc:description`)) {
            fetch_file_2 <- strsplit(x$content$"abstracts-retrieval-response"$coredata$`dc:identifier`, ":")
            if (fetch_file[[1]][2] == fetch_file_2[[1]][2]) {
              if (length(x$content$`service-error`$status$statusCode) == 0 || all("RESOURCE_NOT_FOUND" != x$content$`service-error`$status$statusCode)) {
                ...
                if (length(x$content$"abstracts-retrieval-response"$coredata$`dc:creator`$author[[1]]$`ce:indexed-name`) == 0) {
                  etal[[length(etal) + 1]] <- list("")
                } else
                if (length(x$content$"abstracts-retrieval-response"$coredata$`dc:creator`) == 1) {
                  etal[[length(etal) + 1]] <- list(x$content$"abstracts-retrieval-response"$coredata$`dc:creator`$author[[1]]$`ce:indexed-name`)
                } else {
                  if (is.null(etal_add)) {
                    etal_add <- list("")
                  }
                  etal[[length(etal) + 1]] <- etal_add
                }

                print("ETAL")
                # resea

                if (length(x$content$"abstracts-retrieval-response"$authkeywords$`author-keyword`) == 0) {
                  des[[length(des) + 1]] <- list("")
                } else
                if (length(x$content$"abstracts-retrieval-response"$authkeywords$`author-keyword`) == 1) {
                  des[[length(des) + 1]] <- list(x$content$"abstracts-retrieval-response"$authkeywords$`author-keyword`[[1]])
                } else {
                  des_add <- list("")
                  # if(!is.atomic(xx$content$'abstracts-retrieval-response'$coredata$`dcterms:subject`))
                  # {
                  des_add <- NULL
                  RESULTD <- c(1:length(x$content$"abstracts-retrieval-response"$authkeywords$`author-keyword`))

                  for (d in RESULTD) {
                    if (!is.atomic(x$content$"abstracts-retrieval-response"$authkeywords$`author-keyword`[[d]])) {
                      des_add <- append(des_add, x$content$"abstracts-retrieval-response"$authkeywords$`author-keyword`[[d]][[2]])
                    }
                  } # for
                  # }
                  if (is.null(des_add)) {
                  ...
                  des[[length(des) + 1]] <- des_add
                }

                print("DES")
                ### SUBJE

                if (!is.atomic(x$content$"abstracts-retrieval-response"$coredata$`prism:doi`)) {
                  doi <- append(doi, x$content$"abstracts-retrieval-response"$coredata$`prism:doi`)
                }
...
# save(etal,file="CorpXABV2etal")
# save(des,file="CorpXABV2des")
...
save(CorpX, file = "CorpXABV3_p")
save(etal, file = "CorpXABV3etal_p")
save(des, file = "CorpXABV3des_p")
save(UniX, file = "CorpXABV3UniX_p")
save(YearX, file = "CorpXABV3YearX_p")
save(sco, file = "CorpXABV3sco_p")
save(dv, file = "CorpXABV3dv_p")
